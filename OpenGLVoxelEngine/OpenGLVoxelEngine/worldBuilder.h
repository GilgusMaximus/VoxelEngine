...
public:
	...
	worldBuilder(std::default_random_engine, glm::vec3);
	~worldBuilder();
	...
	std::vector<chunk> calculateVisibleChunks(glm::vec2 cameraPositionXZ);
private:
	...
	FastNoise noise;
	chunk createChunk(int i, int j, int genSize, std::default_random_engine randomEngine, glm::vec3 cameraPosition);
